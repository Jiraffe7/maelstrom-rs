...
impl<Payload> Message<Payload> {
    ...
    where
        Payload: Serialize,
    {
        serde_json::to_writer(&mut *output, self).context("serialise response message")?;
    ...
    IP: Send + 'static,
{
    ...
    .context("init message could not be deserialised")?;
    let InitPayload::Init(init) = init_msg.body.payload else {
        panic!("first message should be init");
    };
    let mut node: N =
        Node::from_init(init_state, init, tx.clone()).context("node initialisation failed")?;
    ...
    serde_json::to_writer(&mut stdout, &reply).context("serialise response to init")?;
    stdout.write_all(b"\n").context("write trailing newline")?;
    ...
    let jh = std::thread::spawn(move || {
        ...
        for line in stdin.lines() {
            ...
            let input: Message<P> = serde_json::from_str(&line)
                .context("Maelstrom input from STDIN could not be deserialised")?;
            if tx.send(Event::Message(input)).is_err() {
                return Ok::<_, anyhow::Error>(());
            }
        }

        let _ = tx.send(Event::EOF).is_err();
        Ok(())
    });
